...
public class LevelManager : MonoBehaviourPun
{
    ...
    private void Start()
    {
        if (PhotonNetwork.LocalPlayer.ActorNumber == 1)
            PhotonNetwork.Instantiate("Prefabs/" + player.name, spawnPoints[0].position, player.transform.rotation);
        else if (PhotonNetwork.LocalPlayer.ActorNumber == 2)
            PhotonNetwork.Instantiate("Prefabs/" + player.name, spawnPoints[1].position, Quaternion.identity);
        else if (PhotonNetwork.LocalPlayer.ActorNumber == 3)
            PhotonNetwork.Instantiate("Prefabs/" + player.name, spawnPoints[2].position, Quaternion.identity);
        else if (PhotonNetwork.LocalPlayer.ActorNumber == 4)
            PhotonNetwork.Instantiate("Prefabs/" + player.name, spawnPoints[3].position, Quaternion.identity);
    }
